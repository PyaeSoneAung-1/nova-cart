...
export default function AdminCategoriesPage() {
  const { data, mutate, isLoading } = useSWR<Category[]>("/categories/admin/list", swrFetcher);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editingId) {
        await api(`/categories/${editingId}`, { method: "PATCH", body });
    ...
    <AdminShell>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{data?.length ?? 0} categories</p>
        </div>
        <Button
          onClick={() => {
            setDraft(EMPTY);
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.map((c) => (
            <Card key={c.id} className="p-5">
              <div className="mb-2 flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <Button
                    ...
                    className="h-8 w-8"
                    onClick={() => {
                      setDraft({ name: c.name, description: c.description ?? "", isActive: c.isActive });
                      setEditingId(c.id);
                    }}
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeleting(c)} aria-label="Delete category">
              ...
              <p className="mt-2 text-xs text-muted-foreground">{c._count?.products ?? 0} products</p>
            </Card>
          ))}
        </div>
      ) : (
      ...
      <Dialog open={draft !== null} onOpenChange={(open) => !open && setDraft(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Organize your products into browsable collections.</DialogDescription>
          </DialogHeader>
          {draft && (
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label>Name</Label>
                <Input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Electronics" />
              </div>
              <div className="space-y-1.5">
      ...
      <AlertDialog open={deleting !== null} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              &quot;{deleting?.name}&quot; will be removed. Categories with products cannot be deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
